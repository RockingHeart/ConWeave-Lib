...
- String（自删除缺陷起）
    ## 添加
        ...
        [1.16.0] 为operator+=添加string_view重载
    ...
    ## 改动
        ...
        [1.14.0] 兼容大小端
        [1.15.1] 更正trimmed的逻辑正确性
        [1.15.2] 更正operator+(char_t)的逻辑正确性
        [1.16.1] 更正分配函数以及已知具有逻辑错误的函数逻辑
        [1.17.0] 将运算符函数迁移至核心接口
    
    ## 保留
        [1.1.6] 参数为(size_t, Option)的resize暂不纳入更新列表，因为日后可能会删除它
        [1.4.0] 以注释的形式保留lower与upper的可读性较高的实现
        [1.8.0] 有了显示this的const说明符推导之后可删除沉余的函数声明的尾部属性，但为了日后扩展，暂不删除相关实现，仅统一接口
